...
update _ keysState (settings, audioChan, cpu) =
  ...
  . (pure . updateTimers <=< emulateCycle . cleanSoundTimer)
  . setKeys keysState
  . CPU.clearKeys
  $ cpu
...
convertGFX graphics = V.map f $ V.filter snd $ V.indexed graphics
...
drawRects graphics renderer = do
  SDL.rendererDrawColor renderer SDL.$= Linear.V4 255 255 255 255
  let newGfx = convertGFX graphics
